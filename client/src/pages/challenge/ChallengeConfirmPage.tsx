...
interface ChallengeConfirmPageProps {
    ...
    <div className="fixed inset-0 flex items-start justify-center bg-[var(--surface-panel)] text-[color:var(--ink)] font-[family-name:var(--font-ui)] overflow-y-auto">
      <div className="w-full max-w-[360px] min-h-full flex flex-col px-[22px] pt-[24px] pb-[22px]">
        ...
        <div className="flex-1 flex flex-col justify-center gap-[26px] px-1">
          <Hero ownerName={ownerDisplayName} />
          <ConfigCard
            boardSize={boardSize}
            timeLimit={timeLimit}
          ...
          <p className="text-small text-[color:var(--ink-muted)] text-center leading-[1.5]">
            One attempt only. After you finish, you'll see how your score stacks up.
          </p>
          <div className="flex flex-col gap-1">
            <StartButton onClick={onStart} />
            <PlayersCount count={playerCount} />
            <button
              type="button"
              onClick={onBack}
...
function Hero({ ownerName }: { ownerName: string }) {
  return (
    <div className="text-center">
      ...
      <div
        className="text-display-sm italic tracking-[-0.015em] font-[family-name:var(--font-display)] truncate px-2 pb-1"
        style={{ fontWeight: 500, color: 'var(--you-accent)', lineHeight: 1.25 }}
        title={ownerName}
      >
        {ownerName}
      </div>
